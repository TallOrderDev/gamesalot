...
class ScalesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create scale" do
    assert_difference('Scale.count') do
      post scales_url, params: { scale: { name: @scale.name, scale_game_id: @scale.scale_game_id } }
    end
  ...
  test "should update scale" do
    patch scale_url(@scale), params: { scale: { name: @scale.name, scale_game_id: @scale.scale_game_id } }
